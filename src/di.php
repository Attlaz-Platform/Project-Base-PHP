...
return [
    \Psr\Log\LoggerInterface::class                 => \DI\factory(function (ContainerInterface $c) {
         ...
         * Log to API
         */
        $attlazApiClient = new \Attlaz\Client('https://api3.attlaz.com', 'democlient', 'democlientsecret');
        $apiLogHandler = new \Attlaz\Project\App\ApiHandler($attlazApiClient);
...
//        $cachePools[] = $mongoDBCache;
        ...
        $arrayCache = new Cache\Adapter\PHPArray\ArrayCachePool(null);
        $cachePools[] = $arrayCache;
...
//        $manager = new \MongoDB\Driver\Manager($mongoDBConnectionString, $mongoDBUriOptions);
        ...
        $cacheManager = new \Attlaz\Project\Model\Cache\CacheManager('attlaz_cache_' . $c->get('branchCode'), $logger);
...
//        $pool = new \Cache\Adapter\MongoDB\MongoDBCachePool($collection);
        ...
        $cacheClient = new \Echron\IO\Client\Cache($arrayCache);
