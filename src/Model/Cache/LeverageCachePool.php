...
class LeverageCachePool implements CacheItemPoolInterface, TaggableCacheItemPoolInterface, LoggerAwareInterface
{
    /**
    ...
    private function handleException($poolKey, $operation, CachePoolException $exception)
    {
        ...
        $this->log('warning', sprintf('Removing pool "%s" from chain because it threw an exception when executing "%s"', $poolKey, $operation), ['exception' => $exception]);

        if ($this->options['remove_pool_on_failure']) {
